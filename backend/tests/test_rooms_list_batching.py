...
def test_the_single_room_endpoints_still_use_the_per_room_path(client):
    """
    The batching is scoped to the LIST endpoint only. GET /api/rooms/{id}
    must keep using room_to_response()/compute_room_navigability()
    unchanged — proved here by the per-room `find_one` fingerprint still
    being present on that route.
    """
    ...
    assert counts["route_edges.find_one"] == 1, dict(counts)
